...
- **The CLI test suite no longer escapes its sandbox on Windows.** The harness pointed the child binary at a temp home via `HOME` — but `os.UserHomeDir` reads `USERPROFILE` on Windows, so install and uninstall tests operated on the real `~/.claude/settings.json` of whoever ran the suite (and left it pointing at a deleted temp binary). Child processes now also receive `USERPROFILE` and an explicit `XDG_CONFIG_HOME`, which outranks the inherited real `%APPDATA%` in config resolution; the built test binary gets its `.exe` suffix; and the tests that depend on chmod-read-only directories — a semantics Windows does not have — skip there. `go test ./...` is green on windows/amd64.
