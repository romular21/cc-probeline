...
- **`[colors]`** — every palette role can be repointed from the config: `green`, `sage`, `amber`, `yellow`, `orange`, `red`, `cyan`, `magenta`. Values are either `"#rrggbb"` (emitted as truecolour) or raw SGR parameters like `"38;5;179"`, which is the escape hatch for anything the terminal understands. An unusable value is skipped rather than applied, so a typo costs one colour rather than the status line. The built-in palette stays 16-colour ANSI so it reads on any terminal theme — a sound default, but a poor straitjacket, since what reads as a calm green on one theme is acid on another.
- **A percentage rendered in place of a bar gets its own colour bands.** A bar earns saturated colour: it is a block of fill and the colour is what gives it meaning. A bare number does not — the same colour reads as an alert when there is nothing to alert about. The healthy band uses `sage` and the notice band `amber` — a calmer green and a softer yellow than the 16-colour ANSI pair, both at full strength. Dimming them was tried first and made the number too faint to read at a glance: the calm has to come from the hue, not from fading the value out. Orange and red are untouched: those are the two that want the eye.
- **`columns`** — sets the layout width outright, for when detection cannot work. Claude Code pipes our stdout, shells do not export `$COLUMNS` to children, and depending on how Claude Code was launched the process may have no controlling terminal either; all three failing means laying out for 80 columns on a terminal that is wider. `cc-probeline diag` prints what each step resolves to, so the guess stops being a guess.
- **`alerts`** — drops the notice row (cache rebuilt, config errors, update available). `tutorial_hints` silences only the rotating tips; this covers the rest. Worth knowing before switching it off: `Cache rebuilt · 60-min idle TTL passed` is what explains a turn that suddenly cost more than the ones around it.
- **`quota_age_note`** — drops the `(as of Xm ago)` suffix on the account-wide `5h`/`7d` block. The suffix appears once the stored snapshot's numbers have gone ten minutes without changing, and it is easy to read as a warning that the figures are stale — but those figures ride in the status-line payload and arrive fresh on every render. What it actually times is how long they have held still, so an idle hour, which spends nothing and therefore changes nothing, raises it beside percentages that are exactly right. Sixteen columns is also enough to push a merged header onto a second row. The per-model figures are the ones that genuinely go stale; their marker is `stale_marker`, and it is unaffected. Default stays true.
...
- **`install --merge-settings` no longer writes a Windows config that silently never renders.** The installer recorded the binary's path the way `os.Executable` spelled it — with backslashes. Claude Code may spawn `statusLine.command` through a POSIX shell (on the machine this was found on, Git Bash), where an unquoted backslash is an escape character: `C:\Users\...` collapses to `C:Users...`, the command is never found, and the failure is fully silent — no error, no log, nothing in `/status`, just an absent line. The path is now normalised to forward slashes, the one spelling accepted by the Windows API, cmd.exe and bash alike; on other platforms this is a no-op, since there a backslash is a legal filename character. The idempotency check compares the normalised form, so re-running `install --merge-settings` over an already-broken config repairs it in place.
- **Log pruning actually happens on Windows now.** `pruneIfNeeded` rewrote the log through a temp file and renamed it over the original — while a deferred `Close` still held the original open. Renaming over an open file is fine on Linux and a sharing violation on Windows, so the rename failed silently on every run and the seven-day prune never removed anything. The source file is now closed before the rename.
- **The CLI/cmd test suites no longer escape their sandbox on Windows.** Three distinct escape mechanisms, each observed live rewriting a real `~/.claude/settings.json` (and leaving it pointing at a deleted temp binary): the harness overrode only `HOME` while `os.UserHomeDir` reads `USERPROFILE` on Windows; `append(os.Environ(), "USERPROFILE="+sandbox)` produced TWO entries and Windows `getenv` takes the FIRST — the real one — where Linux takes the last, which is why the class never showed before; and one test built its install command inline, bypassing the patched helpers entirely. All child env construction now goes through deduplicating helpers (`mergeEnv` / `sandboxEnv`), `%APPDATA%` joined `hermeticStrip` (it is the Windows global-config fallback), the built test binary gets its `.exe` suffix, suffix asserts accept `.exe`, install.sh tests hand bash a forward-slash script path, and chmod-read-only-directory tests skip on Windows where that semantics does not exist. A canary on the real settings.json mtime now guards the claim.

- **One API error no longer kills the status line for the rest of the session.** When a subagent call fails, Claude Code records a synthetic reply — model `<synthetic>`, every usage counter zero — and the parser drops those, since they carry no tokens to count. A sidechain whose only assistant record was such a reply therefore parsed to a transcript with records but no turns, and the activation step indexed the first of zero turns and panicked. A panic means no output at all, and because the record stays in the transcript the line was gone from that session for good: on the machine this was found on, a `529 Overloaded` at 23:16 left the status line blank for the two days that followed. Such a transcript now aggregates to zero, which is what the collector's contract already promised.
- **Width was measured wrong whenever colour was on.** `VisualLen` stripped the marker tokens probes are meant to emit, but not raw ANSI escapes — and the palette exposes raw escape strings that the quota probes concatenate directly, which became unavoidable once colours were user-configurable. A control byte has zero width, but the printable tail of an escape (`[38;5;108m`) counts as ten columns, so every coloured segment measured about eleven columns wider than it drew. `FitLine` compressed segments that still fit and the merged header refused to join rows with room to spare — visible only with colour on, which is precisely what a suite written with `NO_COLOR` cannot catch. The test that pinned the old number said of itself that it documented "runewidth behaviour, not desired output"; it now pins the width the text actually occupies.
- **The status line now lays out for the terminal it is actually in.** Width came from `$COLUMNS` or from asking stdout its size — but Claude Code runs the status-line command with stdout on a pipe so it can capture the output, and `$COLUMNS` is a shell variable that child processes do not inherit. Both checks failed on every render, so the layout was computed for 80 columns no matter how wide the terminal was: segments collapsed to their compact form and a merged header split in two for no visible reason. The controlling terminal is still reachable through `/dev/tty` when the standard streams are redirected, and that is now the third thing tried.
- **The staleness note no longer re-wraps the status line.** A per-model figure Claude Code had stopped refreshing was marked with a trailing `· 2h old`. Those nine columns were enough to push a merged header past the terminal width and split it in two — and since Claude Code refreshes that cache only from certain API responses, the marker is on far more often than off. It is now a single `~` in front of the value, one column wide, at every display level. Switch it off with `stale_marker = false`.
- **A key the setter creates now arrives documented.** Inserting a key wrote a bare `key = value` line, so anyone configuring purely through the CLI accumulated a file of unexplained assignments — `bar_style = "none"` gave no hint that `line`, `low`, `dot` and `block` exist. New keys now carry their accepted values in a trailing comment. Replacing an existing value still leaves the line's comment untouched: whatever the user wrote there is theirs.
- **A `#` inside a config value is no longer mistaken for a comment.** The line-level editor treated the first `#` on a line as the start of an inline comment, which was safe until colours arrived: writing `amber = "#d7a55f"` and then changing it left `amber = "38;5;179" #d7a55f"` behind. Comment detection is now quote-aware.
